...
public class BuffInfo : SkillBase
{
    ...
    public bool CheckDetach()
    {
        ...
        for (var i = 0; i < DetachQualifiedKeyList.Count; i++)
        {
            ...
            {
                // float
                result = Utils.GetCompare<float>(op)(DataScope.GetData<float>(key), Convert.ToSingle(value.Replace("f","")));
            }
            else if (value.EndsWith("L"))
            {
                // long
                result = Utils.GetCompare<long>(op)(DataScope.GetData<long>(key), Convert.ToInt64(value));
            }
            else if (value.Equals("true") && value.Equals("false"))
            {
                // bool
                result = Utils.GetCompare<bool>(op)(DataScope.GetData<bool>(key), Convert.ToBoolean(value));
            }
            else
            {
                // int
                result = Utils.GetCompare<int>(op)(DataScope.GetData<int>(key), Convert.ToInt32(value));
            }
        }

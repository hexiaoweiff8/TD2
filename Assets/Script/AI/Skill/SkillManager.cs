...
public class SkillManager
{
    ...
    public SkillInfo CreateSkillInfo(int skillNum, DisplayOwner skillHolder, int skillLevel = 1)
    {
        ...
        {
            // 从AB文件中加载
            var skillTxt = GetSkillScript(skillNum, RunType);
    ...
    public void AttachSkillAttribute(SkillInfo skillInfo, MemberData memberData)
    {
    ...
    public bool SkillCouldRelease(SkillBase skillbase)
    {
        ...
        if (skillInfo != null)
        {
            if (!CDTimer.Single.IsInCD(skillInfo.Num, skillInfo.ReleaseMember.ClusterData.AllData.MemberData.ObjID.ID, skillInfo.CDGroup))
            {
                result = true;
            }
        }
    ...
    public void SetSkillInCD(SkillInfo skillInfo)
    {
        ...
        {
            // 技能CDGroup
            CDTimer.Single.SetInCD(skillInfo.Num, skillInfo.CDTime, skillInfo.ReleaseMember.ClusterData.AllData.MemberData.ObjID.ID, skillInfo.CDGroup);
        }
    }
    ...
    public void SubSkillCD(SkillInfo skillInfo, float subTime)
    {
        if (skillInfo != null)
        {
            CDTimer.Single.SubCD(skillInfo.Num, skillInfo.CDTime, subTime, skillInfo.ReleaseMember.ClusterData.AllData.MemberData.ObjID.ID);
        }
    }
